...
def train(model, train_optimizer, generator_train,
    ...
    history = model.fit_generator(
        generator_train,
        steps_per_epoch=(int(samples_train // batch)),
        epochs=nb_epochs,
        validation_data=generator_validation,
        validation_steps=(int(samples_validation // batch)))

    print_history(history, freezed, opt_name, lr)


data_dir = '/content/datasets/MIT_400'
work_dir = '/content/datasets/work_unfreeze'
...
learning_rates = [item + abs(np.random.normal(0, item)) for item in np.logspace(-5, -1, 3)]
for ind1, (optimizer_name, optimizer_) in enumerate(optimizers.items()):
    for ind2, learning_rate in enumerate(learning_rates):
        ...
        result = nasnetmob.model.evaluate_generator(test_generator, test_samples)
        print('\nTest loss:', result[0])
